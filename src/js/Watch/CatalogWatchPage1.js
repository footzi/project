...
let CatalogWatchPage1 = React.createClass({
    contextTypes: {
    ...
    handlePreviewClick(productId) {
        this.context.router.push(`/watch-page1/products/${productId}`);
    },

    render() {
        let {  ProductsWatchPage1 } = this.state;
        return (
            <div >
                ...
                <div >
                    {
                        ProductsWatchPage1.map(product =>
                            <ProductPreview
                                key={product.id}
...
export default CatalogWatchPage1;